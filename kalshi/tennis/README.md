...
### Active (wired to Kalshi tickers)
...
| `EXACT_MATCH_NO` | score becomes mathematically impossible mid-match | `KXATPEXACTMATCH` (NO side) |
| `GAME_HANDICAP` | match end only (retirement-safe) | `KXATPGSPREAD` |
...
- **Cold-start tombstones:** When joining a fixture mid-match, end-of-match events are pre-resolved to prevent stale fires. Exception: `first_set_winner` fires at 1-0 cold-start (unambiguous winner) — only tombstoned at 1-1+ or match completed
...
## Daily Mapping Maintenance

Before each trading session, check for unmapped players. Use this prompt with Claude Code:

---

**Check for unmapped Kalshi tennis players and update `kalshi/tennis_mappings.py`.**

Steps:

1. Fetch all active Kalshi tennis markets for today and tomorrow by querying `GET https://api.elections.kalshi.com/trade-api/v2/markets?series_ticker=SERIES&status=open&limit=200` for each series: `KXATPMATCH`, `KXWTAMATCH`, `KXATPCHALLENGERMATCH`, `KXWTACHALLENGERMATCH`. Extract each market's `yes_sub_title` (full player name) and the 3-letter abbreviation from the ticker suffix.

2. Load `kalshi/tennis_mappings.py` and check which Kalshi player names are NOT in `KALSHI_PLAYER_MAP` (match by normalizing names — strip accents, lowercase, remove punctuation).

3. For each unmapped player, determine their V1 alias:
   - Fetch the current V1 tennis catalog (your method for pulling live/upcoming fixtures from Kalstrop V1)
   - Search for the player by surname in the V1 catalog — V1 uses `"Last, First"` format (e.g., `"Draper, Jack"`)
   - If found in V1, use the exact V1 name as the `v1_aliases` value
   - If NOT found in V1, derive the alias as `"Last, First"` from the Kalshi name and mark the entry with `# v1_alias_unconfirmed`

4. Add new entries to `KALSHI_PLAYER_MAP` in alphabetical order:
```python
"draper, jack": {
    "kalshi_name": "Jack Draper",
    "v1_aliases": ["Draper, Jack"],
...
5. Report: how many new players added, how many confirmed vs unconfirmed V1 aliases.

**Important:** For compound surnames (e.g., "Connor Huertas Del Pino"), the V1 alias is NOT simply "Pino, Connor Huertas Del" — V1 uses the full compound surname first (e.g., "Huertas Del Pino, Connor"). Always verify against the V1 catalog when possible.
...
| Set totals timing | Match-end only (Option A, retirement-safe) | Match-end only (retirement-safe) |
| Set handicap | Match-end only (Option A) | Not applicable (Kalshi has no set handicap market) |
...
| Game handicap | Not implemented for tennis | Match-end only (retirement-safe) |
