    ...
    def process_tick(self, fixture_id: str, *, sets_home, sets_away, games_home, games_away,
        ...
        if is_first:
            if state.first_set_completed:
                if state.total_sets >= 2 or state.match_completed:
        ...
        try:
            ...
            for m in r.json().get("markets", []):
                ...
                if BETTING_DATE and BETTING_DATE != "0" and BETTING_DATE not in tk:
                    continue
                # Find matching event by exact code (second segment of ticker)
                tk_parts = tk.split("-")
                if len(tk_parts) >= 2:
                    tk_code = tk_parts[1]
                    entry = code_to_entry.get(tk_code)
                    if entry is not None and tk not in entry.all_tickers:
