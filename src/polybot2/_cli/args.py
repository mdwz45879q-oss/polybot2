...
def add_subcommands(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:  # type: ignore[type-arg]
    ...
    guardian_watch = guardian_sub.add_parser("watch", help="Watch order state and detect overturns")
    ...
    guardian_watch.add_argument("--league", type=str, default="", help="League key for plan loading (e.g., epl, mlb)")
    guardian_watch.add_argument("--link-run-id", type=int, default=None, help="Link run ID for plan loading")
    guardian_watch.add_argument("--bid-threshold", type=float, default=0.80, help="Bid threshold for overturn Signal 2 (default: 0.80)")
    guardian_watch.add_argument("--confirmation-window", type=float, default=10.0, help="Seconds before confirming score reversal (default: 10)")
    guardian_watch.add_argument("--live", action="store_true", help="Execute real cancel/sell orders (default: dry-run)")
