...
def add_subcommands(sub: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:  # type: ignore[type-arg]
    ...
    provider_sync.add_argument("--provider", type=str, nargs="+", choices=["boltodds", "kalstrop_v1", "kalstrop_v2", "kalstrop_opta", "pandascore"], default=[])
    provider_sync.add_argument("--full-catalog", action="store_true", default=False, help="PandaScore: pull full catalog instead of low-latency-only")
    ...
    hotpath_live.add_argument("--refresh-interval", type=int, default=None,
                              help="Seconds between refresh cycles (default: from config/live_trading.py refresh_interval_seconds, or 300)")
    ...
    # --- guardian ---
    guardian_p = sub.add_parser("guardian", help="Order state tracking (snapshot mode)")
    guardian_sub = guardian_p.add_subparsers(dest="guardian_command", required=True)
    guardian_watch = guardian_sub.add_parser("watch", help="Snapshot order state from hotpath log")
    guardian_watch.add_argument("--log-file", type=str, default="", help="Path to hotpath JSONL log file")
    guardian_watch.add_argument("--log-dir", type=str, default="", help="Directory to search for log files")
    guardian_watch.add_argument("--run-id", type=int, default=None, help="Filter log files by run ID")
    guardian_watch.add_argument("--snapshot", action="store_true", help="One-shot: print current state and exit")
    guardian_watch.add_argument("--league", type=str, default="", help="League key for plan loading (e.g., epl)")
    guardian_watch.add_argument("--link-run-id", type=int, default=None, help="Link run ID for plan loading")
    guardian_watch.add_argument("--db", type=str, default="")
